...
def convert_videos(videos, languages=None, language=babelfish.Language('eng'), subtitles_format='.srt', video_format='.mkv', video_savepath=None, single=True, delete_subtitles=False, force_copy=False, force_mkvmerge=False):
    ...
    for video in videos:
        ...
        try:
            ...
            if video_savepath:
                outpath = sort_video(video, video_savepath)
            else:
                outpath = dirpath
...
def sort_video(video, outpath):
    """Try to sort the video according to its type and name

    :param video: video to sort
    :param outpath: root path where the file will be stored
